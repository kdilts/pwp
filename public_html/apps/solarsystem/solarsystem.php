<html>
	...
	<body bgcolor='grey'>
		...
		// Kevin Dilts 2016
			...
			ringCheckAllButton.onclick = function(){
				for(var p in ringBoxes){ ringBoxes[p].checked = true; }
			}

			ringUncheckAllButton.onclick = function(){
				for(var p in ringBoxes){ ringBoxes[p].checked = false; }
			}

			tagCheckAllButton.onclick = function(){
				for(var p in tagBoxes){ tagBoxes[p].checked = true; }
			}

			tagUncheckAllButton.onclick = function(){
				for(var p in tagBoxes){ tagBoxes[p].checked = false; }
			}
			...
			var planets = []; var planetScreenCoords = [];
	...
	var planetData = [
		...
		{ id: 'Eris', semimajor: 67.7, ecc: .441, i: 44*rToD, w: 35.9*rToD }
			...
			for(var p in lines){ scene.add(lines[p]); }
			////
