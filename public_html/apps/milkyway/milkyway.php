<html>
	...
	<body bgcolor = "grey">
		...
		<form style = "position:absolute; left:80%; top:50px">
			...
			<input type="button" value="<-" id="rotateLeft"style = "position:absolute; left:50%; top:116%"></input>
			<input type="button" value="->" id="rotateRight"style = "position:absolute; left:74%; top:116%"></input>
			<input type="button" value="v" id="rotateDown"style = "position:absolute; left:63%; top:126%"></input>
			
			<b1 style = "position:absolute; left:13%; top:97%">Move</b1>
			<input type="button" value="^" id="moveUp"style = "position:absolute; left:15%; top:106%"></input>
			<input type="button" value="<-" id="moveLeft"style = "position:absolute; left:1%; top:116%"></input>
			<input type="button" value="->" id="moveRight"style = "position:absolute; left:25%; top:116%"></input>
			<input type="button" value="v" id="moveDown"style = "position:absolute; left:15%; top:126%"></input>
			
			<b1 style = "position:absolute; left:40%; top:135%">Zoom</b1>
			<input type="button" value="-" id="zoomOut"style = "position:absolute; left:35%; top:145%"></input>
			<input type="button" value="+" id="zoomIn"style = "position:absolute; left:53%; top:145%"></input>
		</form>
		<script>
// Kevin Dilts 2016
window.onresize = function(){
 ...
 Math.randomGaussian = function(mean, standardDeviation) {
 	...
 	if(!mean){ mean = 0.0; }
 	if(!standardDeviation){ standardDeviation = 1.0; }
     ...
     if (Math.randomGaussian.nextGaussian !== undefined) {
         ...
         return (v1 * multiplier * standardDeviation) + mean;
			...
			var sceneWidth = window.innerWidth*.77; var sceneHeight = window.innerHeight*.95;
			...
			// population 1
			var pop1Start = 2; var pop1End = 2;
			var twist = 0; var twistCount = 0;
			...
			// galactic bar
			var barStart = pop1End; var barEnd = barStart;
			...
			// population 2 stars
			var pop2Start = barEnd; var pop2End = pop2Start;
			for(var i = 0; i < 250; i++){
			...
			// open cluster
			var openStart = pop2End; var openEnd = pop2End;
			var twist = 0; var twistCount = 0;
			var alt = 0; var spread = .4;
			for(var z = 0; z < 2; z++){
				for(var r = 1; r < 6; r+=.05){
					for(var t = 0; t < Math.PI*2; t += Math.PI*2/4){
						if(alt == 0){
			...
			// globular
			var globStart = openEnd; var globEnd = globStart;
			for(var i = 0; i < 50; i++){
			...
			// gaseous
			var gasStart = globEnd; var gasEnd = globEnd;
			var twist = 0; var twistCount = 0;
			var alt = 0;
			for(var z = 0; z < 2; z++){
				for(var r = 1; r < 6; r+=.05){
